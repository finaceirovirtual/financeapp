  ...
  const handleSocialLogin = async (provider: () => Promise<void>, providerName: string) => {
    try {
      ...
      if (error instanceof FirebaseError && error.code === 'auth/popup-blocked') {
        toast.error(
          'O popup de login foi bloqueado. Por favor, permita popups para este site e tente novamente.',
          { duration: 5000 }
        );
      } else {
        toast.error(`Falha ao fazer login com ${providerName}. Tente novamente.`);
      }
    } finally {
      setLoading(false);
    }
